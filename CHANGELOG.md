...
- Webhook global config now lives at a single source of truth: `c.WebhookConfigs.Global` aliases `&c.Global.WebhookGlobalConfig`, eliminating the dual-store antipattern that PR [#618](https://github.com/netresearch/ofelia/pull/618) papered over with a hand-rolled `syncGlobalWebhookConfig` copy. Every entry point that mutates the embedded struct (initial INI parse, INI live-reload) is automatically visible to `WebhookManager` without an explicit sync call. INI live-reload of `webhook-allowed-hosts` now also re-runs `WebhookManager.InitManager()` so the URL validator picks up the new whitelist at runtime — previously the data store was refreshed but the security validator stayed snapshotted at startup, so tightening the whitelist via live-reload had no enforcement effect until restart. ([#620](https://github.com/netresearch/ofelia/issues/620))

### Deprecated

- Docker label keys `ofelia.webhooks`, `ofelia.allow-remote-presets`, `ofelia.trusted-preset-sources`, `ofelia.preset-cache-ttl`, and `ofelia.preset-cache-dir` are renamed to their `ofelia.webhook-*` equivalents to match the documented INI `[global]` key names (`webhook-webhooks`, `webhook-allow-remote-presets`, etc.). A user copying their INI keys verbatim into Docker labels previously hit "Unknown global label keys" warnings and silently lost the values for everything except `webhook-allowed-hosts` (the one key that already matched across both paths). The old unprefixed forms still work for backward compatibility but log a one-shot deprecation warning per key — migrate to the `webhook-*` form before the next major release. ([#620](https://github.com/netresearch/ofelia/issues/620))
