...
- Webhook label sync paths now forward all operator-tunable, non-SSRF-sensitive globals into the live config: (a) `mergeWebhookConfigs` propagates `PresetCacheTTL` (precedence: INI value wins over label; the label overrides only when the INI side still holds the documented 24h default, mirroring the existing `AllowedHosts="*"` sentinel), (b) the early-return that skipped global-selector merge when no per-webhook labels were present is split — `ofelia.webhook-webhooks=...` on a service container referencing INI-defined webhooks is no longer dropped, (c) `iniConfigUpdate` now iterates newly added `[webhook "name"]` sections from the reloaded INI and registers them with the webhook manager so a brand-new block takes effect without restart. This also re-enables `webhook-preset-cache-ttl` as a Docker label (it was removed from the allow-list in the [#637](https://github.com/netresearch/ofelia/pull/637) review because the merge path silently dropped it); the key is operationally tunable and not SSRF-sensitive — narrowing or widening a cache TTL cannot widen the network egress surface — so exposing it via labels is a UX win without weakening the [#486](https://github.com/netresearch/ofelia/issues/486) boundary. ([#640](https://github.com/netresearch/ofelia/issues/640), refs [#620](https://github.com/netresearch/ofelia/issues/620), [#637](https://github.com/netresearch/ofelia/pull/637))
