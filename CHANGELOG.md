...
- `[global]` section now recognizes the documented `webhook-*` keys (`webhook-allow-remote-presets`, `webhook-preset-cache-ttl`, `webhook-trusted-preset-sources`, `webhook-preset-cache-dir`, `webhook-allowed-hosts`, `webhook-webhooks`) without emitting "Unknown configuration key" warnings, and the values are now applied to the webhook subsystem ([#604](https://github.com/netresearch/ofelia/issues/604))
