...
func (j *RunServiceJob) Run(ctx *Context) error {
	...
	if isNewService {
		...
		_, err := j.inspectService(ctx, j.Service)
		if err != nil {
			return err
		}
		...
		_, err = j.scaleService(ctx, j.Service, false)
		if err != nil {
			return err
		}

		_, err = j.scaleService(ctx, j.Service, true)
		if err != nil {
			return err
		}
	}

	if err := j.watchContainer(ctx, j.Service); err != nil {
		return err
	}

	if isNewService { // remove created service after completion
		return j.deleteService(ctx, j.Service)
	}
